...
function activate(context) {
    ...
    const compileCommand = vscode.commands.registerCommand("mastermind.compileCurrentFile", async () => {
    ...
    const runCommand = vscode.commands.registerCommand("mastermind.runCurrentFile", async () => {
        const editor = getActiveMmiEditor();
        if (!editor)
            return;
        await editor.document.save();
    ...
    const showBrainfuckCommand = vscode.commands.registerCommand("mastermind.showGeneratedBrainfuck", async () => {
        const editor = getActiveMmiEditor();
        if (!editor)
            return;
        const doc = await vscode.workspace.openTextDocument({
            language: "brainfuck",
            content: `// Placeholder Brainfuck output for:
...
`
    ...
    context.subscriptions.push(output, compileCommand, runCommand, showBrainfuckCommand);
}
function deactivate() { }
